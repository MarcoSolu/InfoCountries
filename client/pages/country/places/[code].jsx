...
const countryPlaces = () => {
  ...
  const fetchPlacesData = async () => {
    try {
      ...
      if (code) {
        ...
        `https://wft-geo-db.p.rapidapi.com/v1/geo/countries/${code}/places`, {
          params: {
  ...
  const elements = countryPlaces.map((place) => {
    return (
        <div key={place.id} className='bg-gray-800 text-white max-w-sm rounded overflow-hidden shadow-lg p-5'>
            <div className='flex flex-col'>
                <h1 className='text-2xl text-center mb-1'>
                    {place.name}
                </h1>
                <p> <strong>Populaion:</strong> {place.population} </p>
